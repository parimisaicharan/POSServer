...
public class MaxLengthCharacterDocument extends PlainDocument {

    private int max;

    public MaxLengthCharacterDocument(int maxLength) {
        max = maxLength;
    }
    ...
    public void insertString(int offset, String str, AttributeSet at) throws BadLocationException {
        ...
        for (int i = 0; i < upper.length; i++) {
            int j = (int) upper[i];
            if ((!Character.isLetter(upper[i]) && (j != 32)) || (getLength() + str.length() > max)) {
