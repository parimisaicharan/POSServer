...
import javax.swing.text.PlainDocument;
 ...
 public class MaxLengthDigitPointDocument extends PlainDocument {
  private int max;

  public MaxLengthDigitPointDocument(int maxLength) {
    max = maxLength;
  }
  ...
  public void insertString(int offset, String str, AttributeSet at) throws BadLocationException {
       ...
       boolean flag = true;
        ...
        for (int i = 0; i < upper.length; i++) {
             int j = (int) upper[i];
            if ((!Character.isDigit(upper[i]) &&  j != 46) || (getLength() + str.length() > max)) {
                flag = false;
